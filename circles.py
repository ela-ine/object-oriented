...
class Triangle:
    def __init__(self, x, y):
        ...
        self.x = x
        self.y = y
        self.x_speed = random.randint(-10,20)
        self.y_speed = random.randint(15,29)
        self.color = '#000000'
    ...
    def draw(self,canvas):
        canvas.create_triangle(self.x,self.y,self.x + self.size, self.y + self.size, fill=self.color, outline="blue")

def addTriangle(event):
    global game_objects # adds to "game_objects" empty list, tells shape to draw itself and add to list
